...
Эта ш8тука нужна чтобы экономить пропускную способность при распространении пакетов, но как - пока не понял

### Config PXE and multicast for OSD ###
Последняя часть модуля, необходимая для распространения ОС. На DP должны быть активированы ___PXE___, ___Multipoint___
